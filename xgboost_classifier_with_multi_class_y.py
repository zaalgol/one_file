...
CSV_PATH = "raw_datasets//IRIS.csv"
OUTPUT_PATH = "output/IRIS_model_output.pkl"
TARGET_COL = "species"
CAT_COLS = []
...
def optimize_params_xgb(trial):
    ...
    return roc_auc_score(y_val, pipeline.predict_proba(X_val), multi_class='ovr')
...
roc_auc = roc_auc_score(y_true, pipeline.predict_proba(df[CAT_COLS + NUM_COLS]), multi_class='ovr')
...
df_with_predictions.to_csv("output/iris_dataset_with_predictions_classifier.csv", index=False)
